...
if (intval($deptID)) {
    ...
    $squery .= "SET @casos2022 = (Select count(*) from cases where codigo_municipio = @deptID AND fecha_registro LIKE '%2022%'), @casos2021 = (Select count(*) from cases where codigo_municipio = @deptID AND fecha_registro LIKE '%2021%'),@casos2020 = (Select count(*) from cases where codigo_municipio = @deptID AND fecha_registro LIKE '%2020%'),@muertos = (Select count(*) from cases where codigo_municipio = @deptID AND recuperado LIKE '%Fallecido%'),@recuperados = (Select count(*) from cases where codigo_municipio = @deptID AND recuperado LIKE '%Recuperado%'),@CasosRegistrados = (Select count(*) from cases where codigo_municipio = @deptID),@CasosMasculinos = (Select count(*) from cases where codigo_municipio = @deptID AND sexo = 'M'), @CasosFemeninos = (Select count(*) from cases where codigo_municipio = @deptID AND  sexo = 'F');";
    ...
    //$squery = "SELECT * FROM `divipola_codes_town` WHERE `codigo_municipio` = " . $deptID;
